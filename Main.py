...
def create_demo_vouch_embed(
    ...
    embed = discord.Embed(
        title="DEMO VOUCH",
        description=(
            "This vouch was generated for demonstration/testing "
            "and is not a genuine user testimonial."
        ),
...
async def handle_vouch_message(
    ...
    if message.content.strip().lower() != "vouch":
        return
...
async def generate_demo_vouches(
    ...
    for index in range(amount):
        ...
        try:
            ...
            print(
                f"Failed to send demo vouch: {exc}"
            )
            return
...
async def handle_vouches_command(
    ...
    # Only intercept -vouches in the three vouch channels.
    if message.channel.id not in VOUCH_CHANNELS:
        return

    # Normal users get the same instruction.
    if not any(
        ...
        await message.channel.send(
            f"{message.author.mention} "
            f"please do `-vouches {{amount}} {{min. Rating}} "
            f"{{max. rating}} {{time}}`"
        )
    ...
    try:
        ...
        await message.channel.send(
            f"{message.author.mention} "
            f"please use numbers for amount, ratings, and time."
        )
        return
    ...
    await message.channel.send(
        f"Starting **{amount} simulated vouches** over "
        f"**{duration} minute(s)**."
    )
